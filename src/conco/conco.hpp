...
struct type_mapper
{
	 ...
	 */
	static T map( storage_type &&value ) noexcept { return static_cast<T>( std::move( value ) ); }
};

// Mapping for l-value references - just forward the reference
template <typename T>
struct type_mapper<T &>
{
	using inner_type = typename type_mapper<T>::inner_type;
	using storage_type = typename type_mapper<T>::storage_type;
	static T &map( T &&value ) noexcept { return value; }
};

// Mapping for const l-value references - just forward the reference as const
template <typename T>
struct type_mapper<const T &>
{
	using inner_type = typename type_mapper<T>::inner_type;
	using storage_type = typename type_mapper<T>::storage_type;
	static const T &map( const T &value ) noexcept { return value; }
};
...
static void apply( context &ctx, auto &callable, auto &&args_tuple )
{
	...
	{
		std::apply( callable, std::move( args_tuple ) );
		if ( !ctx.out.buffer.empty() )
			ctx.out.buffer[0] = '\0';
	}
	else
	{
		auto r = std::apply( callable, std::move( args_tuple ) );
		if ( !ctx.out.buffer.empty() )
