...
struct type_mapper<const char *>
{
	...
	static const char *map( const storage_type &value ) noexcept { return value.c_str(); }
};
...
struct type_mapper<std::vector<T, A>>
{
	...
	static std::vector<T, A> map( const storage_type &value ) noexcept
	  requires !std::is_same_v<typename type_mapper<T>::storage_type, T>
	{
		std::vector<T, A> out;
...
struct type_mapper<std::span<T>>
{
	...
	static span_wrapper<T> map( const storage_type &value ) noexcept
	  requires !std::is_same_v<typename type_mapper<T>::storage_type, T>
	{
		span_wrapper<T> out;
